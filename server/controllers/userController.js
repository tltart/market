...
class UserController {
    async registration(req, res, next) {
        ...
        if (candidate) {
            return next(ApiError.BadRequest("Ползоавтель с таким Email уже есть в системе..."))
        }
        const hashPassword = await bcrypt.hash(password, 5);
        const newUser = await user.create({ email, role, password: hashPassword });
        basket.create({ userId: newUser.id });
    ...
    async login(req, res, next) {
        ...
        if (!email || !password) {
            return next(ApiError.BadRequest("Не заполнено поле Email или пароль"))
        }
        const candidate = await user.findOne({ where: { email } })
        if (!candidate) {
        ...
        return next(ApiError.BadRequest("Не праильный логин или пароль"))
    }
